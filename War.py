...
class Cards:
    def __init__(self):
        ...
        self.values = {'Ace': 14, 'King': 13, 'Queen': 12, 'Jack': 11, '10': 10, '9': 9, '8': 8, '7': 7, '6': 6,
                           '5': 5,'4': 4, '3': 3, '2': 2}
...
class War(Game):
    ...
    def DecideWinner(self):
        ...
        print("Player 0 Card: " + str(winningID.cardForPlayer[1]) + " of " + str(winningID.cardForPlayer[0]))
        print("Player 1 Card: " + str(self.players[1].cardForPlayer[1]) + " of " + str(self.players[1].cardForPlayer[0]))
        self.CheckForTie(winningID, self.players[1])
    ...
    def CheckForTie(self, player0, player1):
        if player0.cardForPlayer[1] == player1.cardForPlayer[1]:
            while player0.cardForPlayer[1] == player1.cardForPlayer[1]:
                player0.cardForPlayer = self.deckOfCards.GetRandomCard()
                player1.cardForPlayer = self.deckOfCards.GetRandomCard()

                print("Player 0 Card: " + str(player0.cardForPlayer[1]) + " of " + str(player0.cardForPlayer[0]))
                print("Player 1 Card: " + str(player1.cardForPlayer[1]) + " of " + str(player1.cardForPlayer[0]))
